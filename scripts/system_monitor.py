...
Columnar format with human-readable sizes and color coding (green/yellow/red).
...
def colorize(value: str, level: str) -> str:
    """level: 'ok' (green), 'warn' (yellow), 'crit' (red)."""
...
# Fixed numeric format: 3 digits before decimal, 1 after (e.g. "  0.1", "100.0") for alignment
...
def get_cgroup_path_from_sys(cgroup_root: Path | None, mem_root: Path | None) -> str | None:
    """Cgroup path (relative to root) from /sys/fs/cgroup only. Uses root of the hierarchy we see.
    In containers, /sys/fs/cgroup is typically the container's cgroup root, so path is ''.
    Avoids /proc/self/cgroup so we are not tied to the Python process."""
...
# ---------------------------------------------------------------------------
# CPU (system-wide from /proc/stat; optional cgroup usage)
# ---------------------------------------------------------------------------
...
def parse_proc_stat_cpu(line: str) -> tuple[float, float, float] | None:
    ...
    if total == 0:
        return None
    usr = 100.0 * (user + nice) / total
    sys_pct = 100.0 * system / total
    idl = 100.0 * idle / total
    return (usr, sys_pct, idl)


def get_cpu_stats() -> tuple[float, float, float]:
    try:
    ...
    return parsed if parsed else (0.0, 0.0, 100.0)
...
def get_cgroup_cpu_usr_sys_idl_from_deltas(
    delta_user_usec: int, delta_system_usec: int
) -> tuple[float, float, float]:
    """(usr%, sys%, idl%) with denominator 1e6 (one second). idl = remainder to 100%."""
    usr = 100.0 * delta_user_usec / USEC_PER_SEC
    sys_pct = 100.0 * delta_system_usec / USEC_PER_SEC
    idl = max(0.0, 100.0 - usr - sys_pct)
    return (usr, sys_pct, idl)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Print dstat-like stats from /proc, /sys and cgroups (columnar, human-readable, color-coded).",
    )
    ...
    ap.add_argument("--interval", "-n", type=float, default=0, help="Refresh every N seconds (0 = single shot)")
    ...
    while True:
        # Cgroup CPU: sample before and after 1s so usr/sys are % of 1e6 usec (one second)
        ...
        # CPU: from cgroup deltas (usr/sys as % of 1e6 usec) or from host
        cpu_src = None
        if in_cgroup and before_cg:
            ...
            if after_cg:
                ...
                usr, sys_pct, idl = get_cgroup_cpu_usr_sys_idl_from_deltas(du, ds)
                cpu_src = "cgroup"
        if cpu_src is None:
            usr, sys_pct, idl = get_cpu_stats()
            cpu_src = "host"

        idl_level = "ok" if idl >= 20 else ("warn" if idl >= 10 else "crit")
        usr_level = "crit" if usr >= 80 else ("warn" if usr >= 60 else "ok")
        sys_level = "crit" if sys_pct >= 50 else ("warn" if sys_pct >= 30 else "ok")
        ...
        if cg_cur is not None:
            # Container view: tot/free/used% from cgroup
            mem_total = cg_max if cg_max is not None else 0
            mem_free = (cg_max - cg_cur) if cg_max is not None else None
            mem_used = cg_cur
            cg_unlimited = cg_max is None
            mem_pct = 100.0 * cg_cur / cg_max if cg_max else 0.0
        else:
            if in_cgroup:
                mem_total, mem_free = get_meminfo()
            mem_used = mem_total - mem_free if mem_total else 0
            mem_pct = 100.0 * mem_used / mem_total if mem_total else 0.0
        mem_level = "crit" if mem_pct >= 90 else ("warn" if mem_pct >= 75 else "ok")
        mem_tot_str = f"{'max':>{NUM_WIDTH}}" if cg_unlimited else human_bytes_short(mem_total)
        mem_free_str = f"{'-':>{NUM_WIDTH}}" if cg_unlimited else (human_bytes_short(mem_free) if mem_free is not None else f"{'-':>{NUM_WIDTH}}")
        ...
        if not args.no_cpu:
            groups.append((f"cpu ({cpu_src})", [
                ("usr", colorize(f"{usr:{NUM_FMT}}%", usr_level)),
                ("sys", colorize(f"{sys_pct:{NUM_FMT}}%", sys_level)),
                ("idl", colorize(f"{idl:{NUM_FMT}}%", idl_level)),
            ]))
        if not args.no_mem:
            groups.append((f"mem ({mem_src})", [
                ("tot", mem_tot_str),
                ("free", colorize(mem_free_str, mem_level)),
                ("used%", colorize(f"{mem_pct:{NUM_FMT}}%", mem_level)),
        ...
        def _io_for_device(dev_name: str) -> tuple[int, int, int, int]:
            """Aggregate disk_deltas for device and its partitions (sda+sda1, nvme0n1+nvme0n1p1)."""
            r_ios = r_sec = w_ios = w_sec = 0
            for k, v in disk_deltas.items():
                if k == dev_name:
                    r_ios += v[0]
                    r_sec += v[1]
                    w_ios += v[2]
                    w_sec += v[3]
                elif k.startswith(dev_name) and len(k) > len(dev_name):
                    suffix = k[len(dev_name):]
                    if suffix.isdigit() or (suffix.startswith("p") and suffix[1:].isdigit()):
                        r_ios += v[0]
                        r_sec += v[1]
                        w_ios += v[2]
                        w_sec += v[3]
            return (r_ios, r_sec, w_ios, w_sec)
        ...
        if first_line or not saved_widths:
            widths = [
                max(min_w, len(sub), len(strip_ansi(val)))
            ...
            while len(widths) < len(columns):
                ...
                widths.append(max(min_w, len(sub), len(strip_ansi(val))))
        ...
        if repeat_header and data_row_count > 0 and data_row_count % repeat_header == 0:
            ...
            for main, subs in groups:
                ...
                col_idx += n
            print(join_with_seps(main_parts, major_between_groups_only=True))
        ...
        # Print sub header row on first output
        sub_parts = [rjust(sub, widths[i]) for i, (_, sub, _) in enumerate(columns)]
        if first_line:
            print(join_with_seps(sub_parts))
