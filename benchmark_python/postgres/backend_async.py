"""PostgreSQL async backend (asyncpg): pool, schema init, batch insert, query."""
...
async def prewarm_pool(pool: asyncpg.Pool, size: int) -> None:
    ...
    # Acquire and release each to warm
    conns = []
    for _ in range(size - 1):
        conns.append(await pool.acquire())
    for c in conns:
        await pool.release(c)
    logger.info("Prewarmed PostgreSQL async pool (%d connections)", size)
...
async def insert_batch(conn: asyncpg.Connection, rows: list[tuple[str, str, str]]) -> int:
    ...
    # Build VALUES with placeholders $1..$N per row
    args: list[Any] = []
